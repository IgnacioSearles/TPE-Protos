...
/**
 * socks5_protocol.h - Definiciones y utilidades del protocolo SOCKS5
 *
 * Implementa todas las constantes, estructuras y funciones de parseo
 * necesarias para el protocolo SOCKS5 según RFC 1928 y RFC 1929.
 *
 * Incluye:
 *  - Constantes del protocolo (versiones, métodos, comandos, códigos)
 *  - Estructuras para mensajes SOCKS5
 *  - Parsers para validación de mensajes entrantes
 *  - Constructores para respuestas del servidor
...
/** Versión del protocolo SOCKS5 */
#define SOCKS5_VERSION 0x05
/** Versión de subnegociación de autenticación */
...
/** Tamaños mínimos de mensajes del protocolo */
...
/** Tamaños de direcciones y componentes */
...
/** Tamaño máximo de un request según RFC 1928 */
#define MAX_REQUEST_SIZE 262

/** Estructura del mensaje de saludo inicial */
typedef struct {
    uint8_t version;
    uint8_t nmethods;
    uint8_t methods[255];
...
/** Respuesta completa del servidor SOCKS5 */
typedef struct {
    uint8_t version;
    uint8_t rep;
    uint8_t rsv;
...
/** Solicitud de autenticación usuario/contraseña */
typedef struct {
    uint8_t version;
    uint8_t ulen;
    char username[255];
...
/** Resultado del parseo del saludo inicial */
typedef struct {
    bool valid;
    uint8_t version;
    uint8_t nmethods;
...
/** Resultado del parseo de autenticación */
typedef struct {
    bool valid;
    bool auth_ok;
    char username[256];
...
/** Resultado del parseo de solicitud */
typedef struct {
    bool valid;
    uint8_t cmd;
    uint8_t atyp;
